...
const LocalKeepDock = preload("res://addons/local_keep_ai/lk_dock.tscn")
var _dock: Control

func _enter_tree() -> void:
	_dock = LocalKeepDock.instantiate()
	add_control_to_dock(DOCK_SLOT_RIGHT_BL, _dock)
	add_tool_menu_item("LocalKeep: Explain Script", _explain_current_script)
	add_tool_menu_item("LocalKeep: Generate GUT Tests", _generate_tests)
	add_tool_menu_item("LocalKeep: List Models", _list_models)

func _exit_tree() -> void:
	if _dock:
		remove_control_from_docks(_dock)
		_dock.free()
	remove_tool_menu_item("LocalKeep: Explain Script")
	remove_tool_menu_item("LocalKeep: Generate GUT Tests")
	remove_tool_menu_item("LocalKeep: List Models")

func _explain_current_script() -> void:
	var script = get_editor_interface().get_script_editor().get_current_script()
	if script == null:
		push_warning("LocalKeep: Open a script first")
		return
